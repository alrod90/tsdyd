...
def add_package():
    try:
        ...
        conn = sqlite3.connect('store.db')
        c = conn.cursor()
        c.execute('INSERT INTO packages (product_id, name, price, is_active) VALUES (?, ?, ?, ?)',
                 (product_id, name, price, is_active))
        conn.commit()
        conn.close()
        return redirect(url_for('admin_panel'))
...
def edit_package():
    try:
        ...
        conn = sqlite3.connect('store.db')
        c = conn.cursor()
        c.execute('UPDATE packages SET name = ?, price = ?, product_id = ? WHERE id = ?',
                 (name, price, product_id, package_id))
        conn.commit()
        conn.close()
        return redirect(url_for('admin_panel'))
    except Exception as e:
        print(f"Error in edit_package: {str(e)}")
        return "حدث خطأ في تعديل الباقة", 500
...
def toggle_package():
    try:
        ...
        conn = sqlite3.connect('store.db')
        c = conn.cursor()
        c.execute('UPDATE packages SET is_active = NOT is_active WHERE id = ?', (package_id,))
        conn.commit()
        conn.close()
        return redirect(url_for('admin_panel'))
    except Exception as e:
        print(f"Error in toggle_package: {str(e)}")
        return "حدث خطأ في تفعيل/تعطيل الباقة", 500
...
def delete_package():
    try:
        ...
        return "حدث خطأ في حذف الباقة", 500
