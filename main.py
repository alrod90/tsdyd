...
async def orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not orders:
        keyboard = [[InlineKeyboardButton("رجوع للقائمة الرئيسية", callback_data='/start')]]
    ...
    for order in orders:
        ...
        keyboard.append([InlineKeyboardButton("رجوع للقائمة الرئيسية", callback_data='/start')]) #added back button
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith('cat_'):
        ...
        if products:
            ...
            keyboard = [[InlineKeyboardButton("رجوع للقائمة الرئيسية", callback_data='/start')]] #added back button
            reply_markup = InlineKeyboardMarkup(keyboard)
...
يمكنك استخدام هذا المعرف للتواصل مع الإدارة.""", reply_markup=reply_markup)
        ...
        try:
            ...
            row = []
            if has_megas:
                row.append(InlineKeyboardButton("استعلام", callback_data=f'megas_{product_id}'))
...
def restore_backup():
    try:
        ...
        # نسخ الملفات إلى المسار الرئيسي
        if os.path.exists(os.path.join(temp_dir, 'store.db')):
            shutil.copy2(os.path.join(temp_dir, 'store.db'), 'store.db')
        if os.path.exists(os.path.join(temp_dir, 'templates')):
            shutil.copytree(os.path.join(temp_dir, 'templates'), 'templates', dirs_exist_ok=True)
