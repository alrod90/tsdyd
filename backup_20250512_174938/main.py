...
async def update_order_status(update: Update, context: ContextTypes.DEFAULT_TYPE, order_id, status):
    ...
    if current_status == 'rejected' and (status == 'pending' or status == 'accepted'):
        ...
        if user_balance < amount:
            ...
            conn.close()
            return
        c.execute('UPDATEusers SET balance = balance - ? WHERE telegram_id = ?', (amount, user_id))
...
def change_order_status():
    try:
        ...
        if new_status == "accepted":
            if 'inquiry' in order.get('note', ''):
                notification_message = f"""✅ رد الخدمة
رقم الطلب: {order_id}
الشركة: {product_name}
المبلغ: {amount} ليرة سوري
الرد: {note if note else 'لا يوجد رد'}"""
            else:
